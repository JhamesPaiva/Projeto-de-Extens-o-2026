...
function bindStaticEventListeners() {
  ...
  document.querySelector('[data-tipo="pf"]').addEventListener('click', () => setTipo('pf'));
  document.querySelector('[data-tipo="pj"]').addEventListener('click', () => setTipo('pj'));

  document.querySelectorAll('[data-toggle-password]').forEach((button) => {
    button.addEventListener('click', () => {
      toggleSenha(button.dataset.togglePassword, button.dataset.iconTarget);
    });
  });

  document.querySelectorAll('[data-password-meter]').forEach((input) => {
    input.addEventListener('input', () => {
      avaliarSenha(input, input.dataset.passwordMeter, input.dataset.passwordLabel);
    });
  });
}
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const loginResult = await authService.login(payload.email, payload.senha);
    console.log('[cadastro.js] loginResult', loginResult);

    authService.setSession(loginResult.user, loginResult.access_token);
